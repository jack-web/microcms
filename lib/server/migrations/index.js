...
exports.migrateToActual = function (cb) {
  async.waterfall([
    ...
    buildMigrationPath,
    function (path, next) {
      async.eachSeries(path, function (item, next) {
        console.log('Migration to version', item.version, 'started...');
        item.migrate(function (err) {
          if (err) return next(err);
          version.create({version: item.version}, next);
          console.log('Migration to version', item.version, 'complete.');
        });
      }, next);

...
app.use(function (req, res, next) {
  req.getFields = function (denyFields) {
    ...
    if (!denyFields) denyFields = [];
    else denyFields = _.isArray(denyFields) ? _.map(denyFields, toLower) : [denyFields.toLowerCase()];

    var fields = req.param('fields');
    if (fields && !_.isArray(fields))
      fields = [fields];
    if (fields) {
      var fields = _.reject(fields, function (f) {
...
/*app.use(i18n.abide({
  supported_languages: ['ru_RU', 'en_US'],
  default_lang: 'en_US',
  translation_directory: 'lib/common/i18n'
}));*/
...
db.on('open', function callback() {
  ...
  initDb(function (err) {
    if (err) return log.error(err, 'Database init error');
  })
});
